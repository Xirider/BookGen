...
def run():
    ...
    parser.add_argument(
        "--save_name", default="processed_1", type=str, required=False
    )
    ...
    for i in range(shard_count):
        ...
        # books = filter_ff_stories(books, max_rating="M", min_words= 100, max_words= 50000, max_chapters= 20, min_chapters= 0, max_books=10000000)
        ...
        if i == shard_count - 1:
            dataset_name = "eval_high_books"
            save_datasets(books, dataset_name, subfoldername=args.save_name)
            dataset_name = "eval"
        else:
            dataset_name = f"train_{i}"

        save_datasets(datasets, dataset_name, subfoldername=args.save_name)
